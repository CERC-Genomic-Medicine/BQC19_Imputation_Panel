...
def compare(sample_name, first_reference_qt_filename, second_reference_qt_filename, path_present_first_missing_second, path_well_first_badly_second, path_badly_first_well_second, path_res, first_reference_name, second_reference_name):
    ...
    with  gzip.open(path_present_first_missing_second, 'wt') as pfms, gzip.open(path_well_first_badly_second, 'wt') as wfbs, gzip.open(path_badly_first_well_second, 'wt') as bfws:
                ...
                for f_chrom, f_pos, f_ref, f_alt, f_imp_gt, f_truth_gt, f_qt in first_variants:
                        ...
                        while second_variants_buffer:
                            ...
                            if s_pos < f_pos:
                                ...
                                break 
    result = {'sample ID':[sample_name], 'present_'+first_reference_name+'_missing_'+second_reference_name:[present_in_first_ref], 'well_'+first_reference_name+'_bad_'+second_reference_name:[well_first_bad_second], 'well_'+second_reference_name+'_bad_'+first_reference_name:[bad_first_well_second]}
    df_re = pd.DataFrame(result)
    df_re.to_csv(path_res, sep='\t', index=False)
